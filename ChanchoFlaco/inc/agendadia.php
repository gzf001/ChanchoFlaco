   <div id="diadia">
  <div class="container">

<div class="row">
  <div class="col-xs-12 yellow btn-back-diadia"><img src="img/flecha-volver.png" alt=""><strong style="font-size: 5vw;">Volver</strong></div>
</div>
                  
     <div class="row">
          <div class="container">
            <div class="col-xs-2"><img  class="img-responsive center-block" src="img/diamante.png" alt=""></div>
            <div id="fechaDiadia" class="col-xs-8 text-center bolder" style="font-size: 6vw; padding-top: 2%;"></div>
            <div class="col-xs-2"><img  class="img-responsive center-block" src="img/diamante.png" alt=""></div>
          </div>
        </div>   
           
      <div class="row ">
        <div class="col-xs-12">
          <img class="img-responsive" src="img/estrella-diadia.png" alt="">
        </div>
    </div>

       

  </div>

   <div class="agregar-diadia distribucion">
        <div class="container">

            <div class="row distribucion-menu">
                     <div class="col-xs-6   text-center gasto-btn-diadia">
                      <img class="btn-gastos center-block img-responsive" src="./img/gasto.png" alt="" >
                      <p  class="txt-blanco" style="font-weight: 800;  font-size: 5vw;">GASTO</p>

                    </div>
                     <div class="col-xs-6  text-center ingreso-btn-diadia">
                      <img class="btn-ingresos center-block img-responsive" src="./img/ingreso.png" alt="" >
                      <p  class="txt-blanco" style="font-weight: 800;  font-size: 5vw;">INGRESO</p>
                    </div>
                </div>
            <!--  <div class="row ">
              <div class="col-xs-8 col-xs-offset-2 boton-agregar-menu">
                <img src="./img/agregar-gasto.png" alt="" class="img-responsive">
              </div>
            </div>-->
          </div> 
 </div>



<div class="container">

  <div class="row distribucion" style="margin-bottom:5%;">
    <div class="col-xs-12 text-center bolder txt-rojo" style=" font-size: 7vw; ">Gastos</div>
  </div>
  <div class="row">
    <div class="col-xs-12"><img  class="img-responsive"src="img/separador-negro.png" alt=""></div>
  </div>

<div id="gastos-diadia">

  <div class="row distribucion">

    <div class="col-xs-6">
      <div class="col-xs-12 text-left bolder" style="font-size: 6vw;">Bebida</div>
      <div class="col-xs-12 text-left " >Alimentos</div>
    </div>

    <div class="col-xs-6 text-right bolder" style="font-size: 6vw;">$1.00000</div>
  </div>  
    <div class="row">
    <div class="col-xs-12"><img  class="img-responsive center-block"src="img/separador-negro.png" alt="" style="opacity:.2;"></div>
  </div>



    <div class="row distribucion">

    <div class="col-xs-6">
      <div class="col-xs-12 text-left bolder" style="font-size: 6vw;">Bebida</div>
      <div class="col-xs-12 text-left " >Alimentos</div>
    </div>

    <div class="col-xs-6 text-right bolder" style="font-size: 6vw;">$1.00000</div>
  </div>  
    <div class="row">
    <div class="col-xs-12"><img  class="img-responsive center-block"src="img/separador-negro.png" alt="" style="opacity:.2;"></div>
  </div>

</div>


  <div class="row distribucion" style="margin-bottom:5%;">
    <div class="col-xs-12 text-center bolder txt-celeste" style=" font-size: 7vw; ">Ingresos</div>
  </div>
  <div class="row">
    <div class="col-xs-12"><img  class="img-responsive"src="img/separador-negro.png" alt=""></div>
  </div>

<div id="gastos-diadia">

  <div class="row distribucion">

    <div class="col-xs-6">
      <div class="col-xs-12 text-left bolder" style="font-size: 6vw;">Bebida</div>
      <div class="col-xs-12 text-left " >Alimentos</div>
    </div>

    <div class="col-xs-6 text-right bolder" style="font-size: 6vw;">$1.00000</div>
  </div>  
    <div class="row">
    <div class="col-xs-12"><img  class="img-responsive center-block"src="img/separador-negro.png" alt="" style="opacity:.2;"></div>
  </div>



    <div class="row distribucion">

    <div class="col-xs-6">
      <div class="col-xs-12 text-left bolder" style="font-size: 6vw;">Bebida</div>
      <div class="col-xs-12 text-left " >Alimentos</div>
    </div>

    <div class="col-xs-6 text-right bolder" style="font-size: 6vw;">$1.00000</div>
  </div>  
    <div class="row">
    <div class="col-xs-12"><img  class="img-responsive center-block"src="img/separador-negro.png" alt="" style="opacity:.2;"></div>
  </div>

</div>



</div>

</div>
